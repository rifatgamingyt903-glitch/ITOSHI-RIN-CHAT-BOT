...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Ew'r Saim",
  description: "Neon spy card",
  commandCategory: "info",
  usages: "[tag/reply/blank]",
  cooldowns: 5
};
...
module.exports.run = async ({ api, event, Users, Currencies }) => {
  try {
    ...
    api.sendMessage("⚡ Generating your neon spy card...", event.threadID, async (err, info) => {
      try {
        const userInfo = await api.getUserInfo(uid);
        const allUsers = await Currencies.getAll(['money','exp']);
        const uDB = await Currencies.getData(uid);

        const infoU = userInfo[uid];
        const genderMap = {
          1: "𝙶𝚒𝚛𝚕 🙋🏻‍♀️",
          2: "𝙱𝚘𝚢 🙋🏻‍♂️",
          0: "𝙶𝚊𝚢 🤷🏻‍♂️"
        ...
        const nickname =
          typeof infoU.alternateName === "string" && infoU.alternateName.trim().length > 0
            ? infoU.alternateName.trim()
            : infoU.name;

        const location = infoU.location?.name || "Private";
        ...
        const username = infoU.vanity || `facebook.com/${uid}`;
        ...
        const buffer = await createSpyCard({
          ...
          uid,
          username,
          gender: genderMap[infoU.gender] || "Unknown",
          type: "User",
          birthday: "Private",
          nickname,
          location,
        ...
        api.sendMessage({ attachment: fs.createReadStream(file) }, event.threadID, () => fs.unlinkSync(file));
      } catch (e) {
        console.log(e);
        api.sendMessage("❌ Failed to generate spy card.", event.threadID);
      }
    });
  } catch (err) {
    console.log(err);
